var Index = require('../lib/invertedIndex.js');
...
describe("Read book data", function(){
	...
	it("Verify non-empty JSON data", function(){
		...
		expect(content).not.toEqual('Unexpected end of input');
	});
...
describe("Populate index", function(){
	...
	it("check if index is created",function(){
		...
		expect(result).toEqual(jasmine.objectContaining({imagination:['0']}));
	});
...
describe("Search index", function(){
	...
	it("should handle index specifiers for search terms",function(){
		...
		expect(searched).toEqual({alice:['0'], powerful:['1'], wonderland:['0']});
	...
	it("should return searches from all indexes",function(){
		...
		expect(searched).toEqual({ books: { alice: [ '0' ], powerful: [ '1' ], wonderland: [ '0' ] },
  														 users: { alice: [ '0' ], powerful: 'not found', wonderland: [ '0' ] } });
	});
